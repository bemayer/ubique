...
assert.deepStrictEqual(reshape([[-1, 3, -1], [4, 5, 9]], 3, 2), [[-1, 3], [
  -1,
  4,
], [5, 9]]);

// Example 4: Reshape a 2x3 matrix into a 3x2 matrix (column-wise)
assert.deepStrictEqual(reshape([[-1, 3, -1], [4, 5, 9]], 3, 2, 1), [[-1, 4], [
  3,
  5,
], [-1, 9]]);

// Example 5: Reshape into a single-column matrix
assert.deepStrictEqual(reshape([[-1, 3, -1], [4, 5, 9]], 6, 1), [
  [-1],
  [3],
  [-1],
  [4],
...
assert.deepStrictEqual(reshape([[-1, 3, -1], [4, 5, 9]], 6, 1, 1), [
  [-1],
  [4],
  [3],
  [5],
