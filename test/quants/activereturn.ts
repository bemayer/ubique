...
assert.deepStrictEqual(activereturn([x, y], z, 12), [[0.041978811047168385], [
  -0.04674575735488551,
]]);
...
assert.strictEqual(
  activereturn(x, y, 52),
  activereturn(x, y, 52, "geometric", 0),
);

// Example 6: Throws error for invalid input
assert.throws(
  () => activereturn(123, y, 12),
  /Input must be an array or matrix/,
);
assert.throws(
  () => activereturn(x, "invalid", 12),
  /Input must be an array or matrix/,
);
