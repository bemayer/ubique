...
const testfun1 = (a: any, b: any, c: any) =>
  a.reduce((sum: any, num: any) => sum + num, 0) / a.length * b + c;
assert.deepStrictEqual(
  vectorfun(0, [[4, 6, 8, 10], [-2, -4, -6, -8]], testfun1, 4, 12),
  [[40], [-8]],
);

// Example 2: Apply function along columns of a matrix
const testfun2 = (a: any, b: any, c: any) =>
  a.reduce((sum: any, num: any) => sum + num, 0) / a.length * b + c;
assert.deepStrictEqual(vectorfun(1, [[5, 6, 5], [7, 8, -1]], testfun2, 5, 10), [
  [40, 45, 20],
]);
...
assert.throws(
  () => {
    vectorfun(2, [[5, 6, 5], [7, 8, -1]], testfun1, 5, 10);
  },
  Error,
  "dimension must be 0 (rows) or 1 (columns)",
);

// Example 5: Missing arguments
assert.throws(
  () => {
    vectorfun(0, [[5, 6, 5], [7, 8, -1]]);
  },
  Error,
...
assert.throws(
  () => {
    vectorfun(0, [[5, 6, 5], [7, 8, -1]], "notAFunction", 5, 10);
  },
  Error,
  "third input argument must be a function",
);

// Example 7: Apply function to a matrix with empty additional arguments
assert.deepStrictEqual(vectorfun(1, [[5, 6, 5], [7, 8, -1]], (a: any) => a), [[
  5,
  6,
  5,
], [7, 8, -1]]);
...
const sumFunction = (a: any) => a.reduce((sum: any, num: any) => sum + num, 0);
